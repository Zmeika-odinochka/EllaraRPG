...
const BOOKSHOP := "res://scenes/bookshop.tscn"
const ROAD := "res://scenes/outskirts.tscn"
const SCENES := [MARKET, CRAFT, TEMPLE, GATES, ARMORY, BOOKSHOP]
const TITLES := {MARKET:"Торговая улица", CRAFT:"Ремесленный квартал", TEMPLE:"Храмовая улица", GATES:"Южные ворота", ARMORY:"Оружейная «У старого клинка»",BOOKSHOP:"Книжная лавка"}
const NPCS := {
	"bookseller": {"name":"Книготорговка Эда", "lines":["Потёртый переплёт дешевле нового. А написано внутри то же самое. Посмотри — может, что и пригодится."]},
...
static func actions(scene: String) -> Array[Dictionary]:
	match scene:
		BOOKSHOP: return [portal(Vector2(384,428),"На торговую улицу",MARKET,Vector2(144,250)),npc("bookseller",Vector2(384,222),Vector2(384,178)),inspect(Vector2(176,284),"Читательский стол","В раскрытом томе лежит сухой лист вместо закладки. Кто-то аккуратно исправил неверную меру верёвки на полях.")]
